...
def significance_plot(SDataframe, BDataframe, methodname, uploadfile, ndots=1000, ROC=True):
	...
	fig.savefig(f"pictures/sig_{uploadfile}.png")
...
def ROC_plot(SEff, BRej, methodname, uploadfile):
	...
	fig.savefig(f"pictures/roc_{uploadfile}.png")
...
def output_hist_plot(STestDataframe, BTestDataframe,
	...
	legend2.AddEntry(STrainHist, "Signal (train sample)", "lpe")
	legend2.AddEntry(BTrainHist, "Background (train sample)", "lpe")
	legend1.Draw()
	legend2.Draw()

	fig.savefig(f"pictures/dist_{uploadfile}.png")
